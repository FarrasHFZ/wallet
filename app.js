/* ============================================
   ...
   + Smart Pattern Recognition & Price Suggestions
   + Week-on-Week Dashboard Card
   + X/Twitter-style Dark Mode
   ============================================ */
...
// ── Firebase SDK (via CDN, loaded as module) ──
...
const Patterns = {
  // Store history keyed by "normalized description" → { amounts: [], category }
  _key: 'wt_patterns',
  ...
  // Normalize: lowercase, remove numbers, trim extra spaces
  normalize(desc) {
    return desc.toLowerCase().replace(/[0-9]/g, '').replace(/\s+/g, ' ').trim();
  },

  // Record a transaction into patterns
  record(desc, amount, category) {
    ...
    all[key].category = category; // update with latest category
    this.saveAll(all);
  },

  // Find suggestion for current input
  suggest(desc) {
    ...
    // Exact match
    if (all[key] && all[key].amounts.length >= 3) {
      return this._buildSuggestion(key, all[key]);
    }

    // Fuzzy: find pattern where key contains words from input or vice versa
    ...
    if (bestMatch) return this._buildSuggestion(bestMatch.key, bestMatch);
    return null;
  },

  _buildSuggestion(key, data) {
...
let accessToken     = null;    // Google OAuth token for Sheets API
let allTransactions = [];
let recentItems     = [];
...
document.addEventListener('DOMContentLoaded', () => {
  // Apply saved theme
  const dark = Config.get('darkMode') !== 'false';
  applyDarkMode(dark);
...
function showApp(user) {
  ...
  // If user is also connected to Google, get Sheets access token
  if (Config.clientId && user.providerData.some(p => p.providerId === 'google.com')) {
    initSheetsAuth(user.email);
  } else if (Config.clientId) {
    // Email/password user — still needs separate Google OAuth for Sheets
    initSheetsAuth(null);
  }
  loadTodayRecent();
}
...
function renderUserAvatar(user) {
  ...
  if (user.photoURL) {
    el.innerHTML = `<img src="${user.photoURL}" style="width:32px;height:32px;border-radius:50%;object-fit:cover">`;
  } else {
...
// ── Google Sheets OAuth (separate from Firebase) ──
function initSheetsAuth(emailHint) {
  if (!Config.clientId) return;
  if (typeof google === 'undefined' || !google.accounts) {
    // GIS script not loaded yet — dynamically load it
    const s = document.createElement('script');
    s.src = 'https://accounts.google.com/gsi/client';
    s.onload = () => initSheetsAuth(emailHint);
  ...
  tc.requestAccessToken({ prompt: '', login_hint: emailHint || '' });
}

// ══════════════════════════════════════════════
// DARK MODE (X/Twitter style)
// ══════════════════════════════════════════════
...
function onDescInput() {
  ...
  // Only show if amount field is empty (don't override what user typed)
  ...
  text.textContent = `💡 Usually Rp ${suggestion.suggestedAmount.toLocaleString('id-ID')} · ${suggestion.category} (${suggestion.count}× before)`;
...
async function sheetsAPI(method, range, body, qp) {
  if (!accessToken) throw new Error('No Sheets access');
...
async function loadTodayRecent() {
  ...
  } catch (e) { console.warn('loadTodayRecent:', e); }
}
  ...
  container.innerHTML = budgetedCats.map(cat => {
    ...
    return `<div class="budget-bar-row">
      <div class="budget-bar-header">
        ...
        <span class="budget-bar-amounts">Rp ${spent.toLocaleString('id-ID')} / Rp ${limit.toLocaleString('id-ID')}</span>
      </div>
      <div class="budget-bar-track"><div class="budget-bar-fill ${status}" style="width:${pct}%"></div></div>
      <div class="budget-bar-pct ${status}">${pct}% ${status==='over'?'🚨 Over budget!':status==='warning'?'⚠️ Almost there':'· Rp '+((limit-spent)).toLocaleString('id-ID')+' left'}</div>
    </div>`;
  }).join('');
}
...
function renderWoWCard(transactions) {
  ...
  const thisWeekTxs = transactions.filter(tx => { const d = parseDate(tx.timestamp); return d && d >= thisWeekStart && d <= now; });
  const lastWeekTxs = transactions.filter(tx => { const d = parseDate(tx.timestamp); return d && d >= lastWeekStart && d <= lastWeekEnd; });
  ...
  // Category breakdown: which cats increased the most
    ...
    bd.innerHTML = `<div class="wow-breakdown-title">Top changes vs last week</div>` +
      deltas.map(({cat, delta: d}) => {
        ...
        return `<div class="wow-breakdown-row">
          ...
          <span class="wow-breakdown-delta ${cls}">${sign}Rp ${Math.abs(d).toLocaleString('id-ID')}</span>
        </div>`;
      }).join('');
  } else {
    bd.innerHTML = '';
...
async function handleSubmit() {
  ...
  try {
    ...
    const monthSpend = allTransactions.filter(tx => tx.category===category && tx.month===curMonth)
...
function buildTransactionItem(tx, index, source) {
  ...
  const bg   = document.createElement('div'); bg.className = 'swipe-delete-bg'; bg.innerHTML = '🗑️';
  ...
  item.innerHTML = `
    ...
    <div class="tx-details">
      <div class="tx-desc">${escHtml(tx.description)}${!tx.sheetRow?' <span style="font-size:10px;color:#f59e0b">⏳</span>':''}</div>
...
function openEdit(source, index) {
  ...
  document.getElementById('edit-category-chips').innerHTML = CATEGORIES.map(cat => {
    ...
    return `<button class="cat-chip ${sel?'selected':''}" style="${sel?`background:${cat.color}`:'background:transparent'}" data-cat="${cat.name}" onclick="selectEditCategory(this,'${cat.name}')">${cat.emoji} ${cat.name}</button>`;
  }).join('');
...
function selectEditCategory(el, name) {
  document.querySelectorAll('#edit-category-chips .cat-chip').forEach(chip => {
    ...
    if (cat?.name===name) { chip.classList.add('selected'); chip.style.background=cat.color; chip.style.color='#fff'; }
    else { chip.classList.remove('selected'); chip.style.background='transparent'; chip.style.color=''; }
  });
}
...
async function loadHistory() {
  if (!accessToken||!Config.sheetId) { showToast('Connect Sheets first','error'); return; }
  const listEl = document.getElementById('history-list');
  listEl.innerHTML = '<p class="empty-state">Loading...</p>';
  try {
    const rows = await readAllRows();
    if (rows.length<=1) { listEl.innerHTML='<p class="empty-state">No data yet</p>'; return; }
...
function renderPieChart(catTotals) {
  ...
  pieChart = new Chart(ctx, {
    ...
    options:{ responsive:true, plugins:{
      legend:{ position:'bottom', labels:{ font:{family:'Inter',size:11}, padding:12, color: document.body.classList.contains('dark')?'#71767b':'#536471' } },
...
function renderBarChart(dailyTotals) {
  ...
  barChart = new Chart(ctx, {
    ...
    options:{ responsive:true,
      ...
      scales:{
        x:{ ticks:{font:{family:'Inter',size:10},maxRotation:45,color:isDark?'#71767b':'#536471'}, grid:{color:isDark?'#2f3336':'#eff3f4'} },
        y:{ ticks:{font:{family:'Inter',size:10},color:isDark?'#71767b':'#536471',callback:v=>v>=1000000?(v/1000000).toFixed(1)+'jt':v>=1000?(v/1000)+'rb':v}, grid:{color:isDark?'#2f3336':'#eff3f4'} },
      },
    },
  });
}
...
async function toggleNotifications() {
  ...
  if (enabled) {
    ...
    updateNotifStatus('ok','✓ Reminders active');
  } else {
    Config.set('notif','false'); clearNotifTimers(); updateNotifStatus('','');
  }
}
...
function fireNotification(title, body) {
  ...
  try {
    const n = new Notification(title, { body, tag:'wallet-tracker', renotify:true });

/* ============================================
   WALLET TRACKER v1.4
   Features: swipe-to-delete, search+filter, dark mode
   ============================================ */
  ...
  for (const [category, keywords] of Object.entries(KEYWORD_MAP)) {
    for (const keyword of keywords) {
      if (input.includes(keyword) && keyword.length > bestLength) {
        bestMatch = category; bestLength = keyword.length;
      }
    }
  }
...
async function loadTodayRecent() {
  ...
  try {
    ...
    recentItems = todayTxs.reverse().slice(0, 10);
    renderRecent();
    updateTodayTotal();
...
function addSwipeToDelete(item, bg, wrapper, onDelete) {
  ...
  const onMove = (x) => {
    if (!isSwiping) return;
    currentX = Math.min(0, x - startX); // only left
  ...
  const onEnd = () => {
    ...
    if (currentX < -THRESHOLD) {
      // Snap to delete position, then trigger
...
function renderRecent() {
  ...
  el.innerHTML = '';
  recentItems.forEach((tx, i) => {
    el.appendChild(buildTransactionItem(tx, i, 'recent'));
  });
}

// ============================================
// SWIPE DELETE — CONFIRM FLOW
// ============================================
...
function openDelete(source, index) {
  ...
  document.getElementById('delete-preview').innerHTML =
    `<strong>${escHtml(tx.description)}</strong><br>
     Rp ${tx.amount.toLocaleString('id-ID')} · ${tx.category}<br>
     <span style="font-size:12px;opacity:0.7">${tx.timestamp}</span>`;
...
async function confirmDelete() {
  ...
  if (tx.sheetRow) {
    try {
      ...
      // After deletion, sheet rows shift — re-index
      allTransactions.forEach((t, i) => {
        if (t.sheetRow > tx.sheetRow) t.sheetRow -= 1;
      });
...
function closeDelete() {
  ...
  document.querySelectorAll('.swipe-delete-bg').forEach(el => {
    el.style.width = '0';
  });
}

function closeDeleteIfOverlay(event) {
  if (event.target === document.getElementById('delete-modal')) closeDelete();
}
...
function selectEditCategory(el, name) {
  document.querySelectorAll('#edit-category-chips .cat-chip').forEach(chip => {
    ...
    if (cat && cat.name === name) {
      chip.classList.add('selected'); chip.style.background = cat.color; chip.style.borderColor = cat.color;
    } else {
      chip.classList.remove('selected'); chip.style.background = ''; chip.style.borderColor = '';
    }
  });
}
...
async function saveEdit() {
  ...
  tx.amount = newAmount; tx.category = newCategory;
  tx.month = newMonth; tx.week = newWeek;
...
function applyHistoryFilter() {
  activeSearchQuery = document.getElementById('history-search')?.value.toLowerCase().trim() || '';
  const filtered = allTransactions.filter(tx => {
    const matchCat = activeFilterCat === 'all' || tx.category === activeFilterCat;
    const matchSearch = !activeSearchQuery || tx.description.toLowerCase().includes(activeSearchQuery);
  ...
  if (activeFilterCat !== 'all' || activeSearchQuery) {
...
function renderHistoryList(el, list) {
  ...
  if (txList.length === 0) {
    el.innerHTML = '<p class="empty-state">Tidak ada transaksi ditemukan</p>';
    return;
  }
  el.innerHTML = '';
  let lastDate = '';
  txList.forEach((tx, i) => {
    ...
    // Find original index in allTransactions for proper edit/delete reference

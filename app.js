...
async function showApp(user) {
  ...
  transactionsTabVerified = false; // re-verify tab for this user's Sheet
  sheetsTokenWaiters.forEach(w => w.reject(new Error('User changed')));
  sheetsTokenWaiters = [];

  // Init Sheets OAuth client — await so token client is ready before any API call
  if (Config.clientId) {
    const hint = user.email || '';
    await initSheetsClient(hint); // now returns Promise
  }
...
async function handleSignOut() {
  ...
  sheetsTokenClient      = null;
  sheetsTokenPending     = false;
  transactionsTabVerified = false; // re-verify for new user's Sheet
...
// Called before any read/write to the Sheet.
// If "Transactions" tab is missing, creates it with headers automatically.
// User never needs to manually set up the Sheet.
// ══════════════════════════════════════════════

let transactionsTabVerified = false; // cache so we only check once per session

async function ensureTransactionsTab() {
  if (transactionsTabVerified) return; // already confirmed this session

  const token   = await ensureToken();
  const baseUrl = 'https://sheets.googleapis.com/v4/spreadsheets/' + Config.sheetId;

  // Fetch spreadsheet metadata to check existing tabs
  const metaRes = await fetch(baseUrl, {
    headers: { 'Authorization': 'Bearer ' + token },
  });
  if (!metaRes.ok) throw new Error('Cannot access Sheet. Check Sheet ID in Settings.');

  const meta      = await metaRes.json();
  const sheets    = meta.sheets || [];
  const existing  = sheets.find(sh => sh.properties?.title === 'Transactions');

  if (existing) {
    // Tab exists — check if headers are there, add if missing
    try {
      const headerRes = await fetch(
        baseUrl + '/values/Transactions!A1:G1',
        { headers: { 'Authorization': 'Bearer ' + token } }
      );
      ...
      if (firstRow.length === 0) {
        // Empty — write headers
        await writeTransactionsHeaders(token, baseUrl);
      }
    } catch (e) { /* headers check failed, proceed anyway */ }
    transactionsTabVerified = true;
    return;
  }
  ...
  transactionsTabVerified = true;
  console.log('Transactions tab created automatically');
}

async function writeTransactionsHeaders(token, baseUrl) {
  await fetch(
    baseUrl + '/values/Transactions!A1:G1?valueInputOption=USER_ENTERED',
    {
      method: 'PUT',
      headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
function initSheetsClient(emailHint) {
  // Returns a Promise that resolves when token client is ready
  return new Promise((resolve) => {
    ...
    const tryInit = () => {
      ...
      // Always re-create client so new user gets fresh hint
...
function ensureSheetsToken() {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Token client should be ready (initSheetsClient awaited in showApp)
    // But handle edge case where it's still null (no Client ID, or GIS blocked)
    if (!sheetsTokenClient) {
      sheetsTokenPending = false;
      const waiters = sheetsTokenWaiters.splice(0);
      waiters.forEach(w => w.reject(new Error(
        'Google Sheets not connected. Add Client ID in Settings and re-login.'
      )));
      return;
    }
...
async function appendRow(row) {
  await ensureTransactionsTab(); // create tab if missing
  const token = await ensureToken();
...
async function handleSubmit() {
  ...
  try {
    ...
    const msg = e.message || '';
    // Only queue offline for real network issues, not auth/config issues
    if (!navigator.onLine || msg.includes('Failed to fetch') || msg.includes('NetworkError')) {
      Queue.add(txData);
      showToast('Offline — saved to queue, will sync when connected', 'info');
    } else if (msg.includes('not connected') || msg.includes('Client ID') || msg.includes('Sheet ID')) {
      // Config issue — tell user what to fix, don't queue
      showToast('⚠ ' + msg, 'error');
    } else {
      // Unknown error — queue it to be safe
      Queue.add(txData);
      showToast('Save failed — queued. Error: ' + msg, 'error');
    }
  } finally {
    btn.disabled=false; btnText.textContent='Add Spending'; btnLoader.classList.add('hidden');
  }

...
document.addEventListener('DOMContentLoaded', () => {
  ...
  firebaseAuth.getRedirectResult().then(result => {
    // result.user is null if no redirect was in progress — that's fine
  }).catch(e => {
  ...
  // Firebase auth state listener — this is what drives show/hide of auth vs app
...
async function signInGoogle() {
  ...
  try {
    if (isMobile) {
      ...
      // Page will reload; onAuthStateChanged catches the result
    } else {
    ...
    if (e.code !== 'auth/popup-closed-by-user') {
      showAuthError('login', friendlyAuthError(e.code));
      showToast('Google sign-in failed — check console', 'error');

...
async function setupUserSheet(user) {
  ...
  try {
    ...
    // ── Priority 1: Firestore has Sheet ID (returning user or already migrated) ──
    if (docSnap.exists && docSnap.data().sheetId) {
      ...
      console.log('Sheet loaded from Firestore:', sheetId);
      return;
    }

    // ── Priority 2: localStorage has Sheet ID (existing user, not yet migrated) ──
    // Migrate it to Firestore so it's persistent and account-linked
    const existingSheetId = Config.get('sheetId');
    if (existingSheetId) {
      console.log('Migrating existing Sheet ID to Firestore:', existingSheetId);
      await docRef.set({
      ...
      updateSheetStatus('connected');
      showToast('✓ Your existing Sheet has been linked to your account', 'success');
      return;
    }

    // ── Step 2: New user — need Google token to create Sheet ──
    // Request token now (user just logged in so gesture chain is warm)
    updateSheetStatus('creating');

    // Wait for Sheets token (user needs to have connected Google)
    let token;
    try {
      token = await ensureToken();
    } catch (e) {
      // No Google OAuth yet — prompt user to connect
      updateSheetStatus('needs_setup');
      return;
    }

    // ── Step 3: Create a new Google Sheet via Drive API ──
    ...
    if (!createRes.ok) {
      ...
      throw new Error('Could not create Sheet: ' + (err.error?.message || createRes.status));
    }

    const file    = await createRes.json();
    const sheetId = file.id;

    // ── Step 4: Set up headers in the new Sheet ──
    await fetch(
    ...
    // Rename Sheet1 tab to "Transactions"
          ...
          body: JSON.stringify({
            requests: [{
              updateSheetProperties: {
                properties: { sheetId: tabId, title: 'Transactions' },
                fields: 'title',
              },
            }],
          }),
    ...
    // ── Step 5: Save Sheet ID to Firestore and localStorage ──
    await docRef.set({ sheetId, email: user.email || '', createdAt: new Date().toISOString() });
    Config.set('sheetId', sheetId);
    updateSheetStatus('connected');

    showToast('✓ Your personal Wallet Tracker Sheet has been created!', 'success');
    console.log('New Sheet created and saved:', sheetId);

  } catch (e) {
    console.error('setupUserSheet error:', e);
    updateSheetStatus('error');
    showToast('Sheet setup failed: ' + e.message, 'error');
  }
}

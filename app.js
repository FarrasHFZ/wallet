...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // onAuthStateChanged handles BOTH normal sign-in AND after Google popup/redirect
...
function signInGoogle() {
  // IMPORTANT: signInWithPopup() must be called synchronously from the click handler.
  // Any await before this line breaks the "trusted user gesture" chain on mobile Chrome,
  // which causes the popup to be silently blocked.
  // We also do NOT use signInWithRedirect because on GitHub Pages (static host)
  // the Firebase compat SDK cannot reliably recover the redirect result —
  // it shows the login screen again after redirect (the bug being fixed here).
  clearAuthErrors();
  ...
  firebaseAuth.signInWithPopup(googleProvider).then(() => {
    // onAuthStateChanged will fire → showApp() — nothing to do here
  }).catch(e => {
    ...
    if (e.code === 'auth/popup-blocked') {
      showAuthError('login', 'Popup blocked. Tap the address bar in Chrome → tap "Always allow popups" → try again.');
      showToast('Allow popups in Chrome, then retry', 'error');

...
function renderBudgetInputs() {
  ...
  container.innerHTML = CATEGORIES.map(cat => `
    <div class="budget-input-row">
      <span class="budget-input-label">${cat.emoji} ${cat.name}</span>
      <input type="text" class="budget-input-field" id="budget-${cat.name.replace(/[^a-z]/gi,'_')}"
        inputmode="numeric" placeholder="Tidak ada limit"
        value="${budgets[cat.name] ? Number(budgets[cat.name]).toLocaleString('id-ID') : ''}">
    </div>
  `).join('');
  ...
  container.querySelectorAll('.budget-input-field').forEach(inp => {
    inp.addEventListener('input', () => {
      ...
      if (raw) inp.value = Number(raw).toLocaleString('id-ID');
      else inp.value = '';
    });
  });
}

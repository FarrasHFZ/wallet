...
function showApp(user) {
  ...
  // Initialise Sheets token client (no token request yet — lazy on first API call)
  if (Config.clientId) {
    const hint = user.providerData?.some(p => p.providerId === 'google.com')
      ? user.email
      : Config.get('sheetsHint') || '';
...
function ensureSheetsToken() {
  ...
  // If token is still valid, resolves immediately.
  // If not, requests one silently (no popup) and queues the resolve.
  return new Promise((resolve, reject) => {
    ...
    if (!Config.clientId || !sheetsTokenClient) {
      reject(new Error('Google Sheets not configured. Add Client ID in Settings.'));
      return;
    }
    // Queue this waiter
    ...
    // Silent token request — prompt:'' means NO popup, NO UI ever
    // GIS resolves this from cached consent granted during first Sheets use
    sheetsTokenClient.requestAccessToken({
      prompt:     '',   // ← key: empty = silent, no UI shown to user

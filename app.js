...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Firebase auth state — single source of truth for login/logout
  firebaseAuth.onAuthStateChanged(user => {
    if (user) {
      ...
      currentUser = null;
      showAuthScreen();
      // Init Google One Tap after auth screen is shown
      initGoogleOneTap();
    }
  });
});
...
// ── Google One Tap — works natively on mobile without popup/redirect ──────────
// On mobile: shows Android native account chooser (no popup, no redirect)
// On desktop: shows One Tap overlay, fallback to popup
function initGoogleOneTap() {
  if (typeof google === 'undefined' || !google.accounts) {
    setTimeout(initGoogleOneTap, 600);
    return;
  }
  const oauthClientId = Config.clientId || '';
  if (!oauthClientId) return; // No client ID set yet
  ...
  google.accounts.id.initialize({
    ...
    context: 'signin',
    ux_mode: 'popup', // keeps it in-page, no redirect
  });

  // Prompt One Tap (native bottom sheet on Android Chrome)
  ...
  // Render proper Google button inside container divs
...
function handleOneTapResponse(response) {
  // Google returns an ID token — exchange it for Firebase credential
...
// Called when user taps the "Continue with Google" button manually
function signInGoogle() {
  clearAuthErrors();

  // Re-trigger One Tap prompt first (most reliable on mobile)
  if (typeof google !== 'undefined' && google.accounts && google.accounts.id && Config.clientId) {
    google.accounts.id.prompt(notification => {
      if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
        // One Tap suppressed (e.g. user dismissed it before) — fall back to popup
        doPopupSignIn();
      }
      // else: One Tap is showing — user will pick account from bottom sheet
    });
    return;
  }

  // GIS not available — straight to popup
  doPopupSignIn();
}

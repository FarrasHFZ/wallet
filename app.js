...
let barChart = null;
let editTarget = null;
let useCustomDate = false; // false = Hari Ini, true = custom date
...
function getSelectedDate() {
  ...
  // Parse YYYY-MM-DD and set current time
...
// ============================================
// GOOGLE AUTH (with auto-reconnect)
// ============================================
...
function tryAutoSignIn() {
  ...
  const attempt = () => {
    ...
    tokenClient.requestAccessToken({
      prompt: '',
      login_hint: Config.get('userEmail'),
    });
  };
  attempt();
}
...
function onTokenResponse(resp) {
  ...
  fetchAndStoreEmail();
  if (!Config.get('userEmail')) {
    showToast('Signed in ✓', 'success');
  }
  updateAuthUI(true);
  // Load today's transactions for the recent section
  loadTodayRecent();
}
...
// ============================================
// LOAD TODAY'S RECENT (fixes empty list on reload)
// ============================================
...
async function loadTodayRecent() {
  ...
  try {
    ...
    // Filter for today: compare the date part of timestamp
    const todayTxs = all.filter(tx => {
      const txDate = tx.timestamp.split(',')[0]?.trim() || tx.timestamp.split(' ')[0] || '';
      return txDate === todayStr;
    });

    // Show most recent first, max 5
    recentItems = todayTxs.reverse().slice(0, 5);
    renderRecent();
    updateTodayTotal();

    // Also cache for history/dashboard
    allTransactions = all.reverse();
  } catch (e) {
    console.warn('Could not load today recents:', e);
  }
}
...
function updateTodayTotal() {
  ...
  const amountEl = document.getElementById('today-total-amount');

  if (total > 0) {
    badge.classList.remove('hidden');
    amountEl.textContent = 'Rp ' + total.toLocaleString('id-ID');
  } else {
...
function renderRecent() {
  ...
  if (recentItems.length === 0) {
    el.innerHTML = '<p class="empty-state">📭 Belum ada pengeluaran</p>';
    return;
  }
  el.innerHTML = recentItems.map((tx, i) => {
    const cat = CATEGORIES.find(c => c.name === tx.category) || CATEGORIES[9];
    return `<div class="transaction-item" style="border-left-color:${cat.color}" onclick="openEdit('recent', ${i})">
...
function openEdit(source, index) {
  ...
  // Parse the timestamp to set the date picker
  // Timestamp format: "DD/MM/YYYY, HH:MM:SS" (id-ID locale)
...
async function saveEdit() {
  ...
  // Build new timestamp from the date picker
  let newTimestamp = tx.timestamp;
  let newMonth = tx.month;
  let newWeek = tx.week;

  if (newDateVal) {
    const [y, m, d] = newDateVal.split('-').map(Number);
    const editDate = new Date(y, m - 1, d);
    // Keep the time from original or use current
    const timePart = tx.timestamp.split(',')[1]?.trim() || new Date().toLocaleTimeString('id-ID');
    newTimestamp = editDate.toLocaleDateString('id-ID', {
      year: 'numeric', month: '2-digit', day: '2-digit',
    }) + ', ' + timePart;
  ...
  if (editTarget.sheetRow) {
    try {
      const row = [newTimestamp, newDesc, newAmount, newCategory, newMonth, newWeek];
...
function renderHistoryList(el) {
  ...
  allTransactions.forEach((tx, i) => {
    const dateStr = tx.timestamp.split(',')[0]?.trim() || tx.timestamp.split(' ')[0] || '';
    if (dateStr !== lastDate) {
      html += `<div class="history-date-group">${dateStr}</div>`;
      lastDate = dateStr;
    }
    const cat = CATEGORIES.find(c => c.name === tx.category) || CATEGORIES[9];
    html += `<div class="transaction-item" style="border-left-color:${cat.color}" onclick="openEdit('history', ${i})">
      <span class="tx-emoji">${cat.emoji}</span>
      <div class="tx-details">
        <div class="tx-desc">${escHtml(tx.description)}</div>
        <div class="tx-cat">${tx.category}</div>
...
function renderDashboardData() {
  ...
  const dailyTotals = {};
  filtered.forEach(tx => {
    const day = tx.timestamp.split(',')[0]?.trim() || tx.timestamp.split(' ')[0] || 'unknown';

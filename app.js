...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Start tutorial for new users (skipped if already completed)
...
// Google sign-in removed — email/password only on mobile
// (Google popup/redirect both fail on mobile Chrome static hosting)
// Google account is still used for Sheets API via initSheetsAuth()
...
// Uses prompt:'' (no UI) for silent refresh — never shows a popup to the user
// Google silently returns a token if user previously granted access
// Token lasts ~1hr; we auto-refresh in background when it expires
  ...
  const tryInit = () => {
    ...
    if (!sheetsTokenClient) {
      sheetsTokenClient = google.accounts.oauth2.initTokenClient({
        ...
        error_callback: (err) => {
          // Silent fail — Sheets auth is optional (for offline-first users)
          console.warn('Sheets silent auth failed:', err);
        },
      });
    }

    // Always use prompt:'' — this is a SILENT request, no popup ever shown
    // Google returns token immediately from cached consent (granted previously)
    sheetsTokenClient.requestAccessToken({
      prompt:     '',          // empty string = silent, no UI
      login_hint: emailHint || Config.get('userEmail') || '',
    });
  };
  tryInit();
}
...
function onSheetsToken(resp) {
  if (resp.error) {
    console.warn('Sheets token error:', resp.error);
    return;
  }
  accessToken     = resp.access_token;
  // Token expires in ~3600s, refresh 5min early
  ...
  setTimeout(() => {
    if (currentUser && Config.clientId) initSheetsAuth(currentUser.email);
  }, ((resp.expires_in || 3600) - 300) * 1000);
}

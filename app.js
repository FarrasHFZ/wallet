...
document.addEventListener('DOMContentLoaded', () => {
  loadSettingsUI();
  tryInitGoogle();
  // Auto-reconnect if user has signed in before
  tryAutoSignIn();
...
function tryAutoSignIn() {
  // Only auto-sign-in if user has signed in before
  if (!Config.clientId || !Config.get('userEmail')) return;

  isAutoSigningIn = true;
  document.getElementById('auth-status').textContent = '⟳ Menghubungkan...';

  // Wait for GIS library + tokenClient to be ready
  const attempt = () => {
    if (!tokenClient) {
...
function onTokenResponse(resp) {
  ...
  // Fetch and store user email for future auto-sign-in
  fetchAndStoreEmail();
...
async function fetchAndStoreEmail() {
  try {
    ...
    if (data.email) {
      Config.set('userEmail', data.email);
    }
  } catch (e) {
...
function handleSignOut() {
  ...
  accessToken = null;
  Config.set('userEmail', ''); // Clear stored email so auto-sign-in won't trigger
  updateAuthUI(false);
